...
export class UIManager {
  ...
  showDialog(sequence, index = 0) {
    return new Promise((resolve) => {
      ...
      // v2: choices may carry `when: (state) => bool` for conditional availability
      const dialogState = this.gameState.getState();
      const visibleChoices = step.choices.filter(c => !c.when || c.when(dialogState));
      for (const choice of visibleChoices.length ? visibleChoices : step.choices) {
        ...
        btn.onclick = () => {
          ...
          // v2: optional callback when this choice is picked
